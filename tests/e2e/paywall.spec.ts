...
test('пейволл — не тупик: крестик и Esc выводят с экрана', async ({ page }) => {
  // Фидбэк тестеров 14.07: с пейволла не было выхода (модалка без close на вебе).
  await login(page);
  await openPaywall(page);

  // Крестик в хедере: закрывает даже при ПРЯМОМ заходе (истории нет → replace на /).
  await page.getByLabel('Close').first().click();
  await expect(page).not.toHaveURL(/paywall/, { timeout: 10_000 });

  // Esc — привычный выход на десктопе.
  await page.goto('/paywall');
  await expect(page.getByText(/Learn faster with Premium/i)).toBeVisible({ timeout: 20_000 });
